...
## VBO v2: every commit/attest requires a signed Atlas payload (found Sept 23, 2026)
Despite the inline comment ("lenient — returns 0,0 if not present"), `_getVerifiedFeedDataLenient` calls `_authenticateAndUnpackExtraData()` first, which reverts (`InsufficientMetadata` / `InvalidMarker`) when no Atlas payload is appended to calldata. Only an *unmatched feed* inside a valid payload returns 0. Consequences: (1) commits cannot be made without a live Atlas payload; (2) VBO v2 reads BTC/USD (#626) only — for non-BTC commitments the frontend appends that asset's feed (ETH #852 / SOL #635) so priceAtCommit is stored as 0 rather than BTC's price. VBO v3 should take the feed ID per commitment and make the no-payload path explicit.
